...
**Simple single-line prompti only (for now):**
...
Agent: Complete. Report: workspace/results/quantum_computing_summary_20251115.txt
```
...
- **Responsibilities**:
  ...
  - Creates comprehensive reports in `workspace/results/`
...
│   └── skills/            # Custom skills for agents (optional)
│       └── README.md      # Skills documentation and examples
│
...
│   ├── research-notes/   # Raw research findings
│   └── results/          # Final synthesized reports
│
...
Skills are optional modular knowledge packages for specialized formatting or domain expertise. To create a skill:

```bash
mkdir -p .claude/skills/your-skill-name
# Create SKILL.md with guidelines, templates, or instructions
```

Agents can then reference skills in their prompts:
```
...
Edit `agents/agent.py` to customize:
...
- The `workspace/` directory should be created automatically
- Check session logs for errors: `cat logs/session_*/transcript.txt`


## Contributing

This is a standalone template project. Feel free to:
- Add new subagent types
- Enhance existing prompts
- Create new skills
- Fork it, copy it, do whatever you want to do with it! 


## Roadmap

- Add a better UI that allows for multi-line prompts and multi-modal sharing (e.g. images, files, etc)
- Improve tracking and logging
- Add usage monitoring and token controls (i.e. rate limiting)
- Move agent definitions into .claude for standard *.md format with an "agent_loader" tool
- Add new development tooling for skills, dynamic agents, connecting to marketplaces, etc.
...
- [Example Research Reports](./workspace/results/) (after running)
